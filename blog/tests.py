...
class TestView(TestCase):
    def setUp(self):
        ...
        self.user_rlguswn.is_staff = True
        self.user_rlguswn.save()
    ...
    def test_create_post(self):
        ...
        self.client.login(username='Kase', password='somepassword')
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)

        self.client.login(username='rlguswn', password='somepassword')
        response = self.client.get('/blog/create_post/')
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
    ...
    def test_comment_form(self):
        ...
        self.assertIn('kase', new_comment_div.text)

    def test_comment_update(self):
        comment_by_rlguswn = Comment.objects.create(
            post=self.post_001,
            author=self.user_rlguswn,
        ...
        self.assertIn('Update comment', comment_001_div.text)
        self.assertIn('Updated: ', comment_001_div.text)

    def test_delete_comment(self):
        comment_by_kase = Comment.objects.create(
            post=self.post_001,
            author=self.user_kase,
            content="kase's comment"
        )
        ...
        self.assertIn(self.post_001.title, soup.title.text)
        comment_area = soup.find('div', id='comment-area')
        self.assertNotIn("kase's comment", comment_area.text)
